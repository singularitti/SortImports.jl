...
[`SortImports`](https://github.com/singularitti/SortImports.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("SortImports")
   ```

3. Run

   ```julia-repl
   julia> using SortImports
   ```
...
version of `SortImports`, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/SortImports.jl"
```
...
our [GitHub repository](https://github.com/singularitti/SortImports.jl)
for new releases.
Once we release a new version, you can update `SortImports` by typing

```@repl
using Pkg
Pkg.update("SortImports")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall `SortImports`
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("SortImports")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall `SortImports`.
