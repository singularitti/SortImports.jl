...
!!! info
    The style guide is always a work in progress, and not all SortImports code
    follows the rules. When modifying SortImports, please fix the style violations
...
`SortImports` uses [`JuliaFormatter`](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/SortImports.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the `SortImports` directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to `SortImports` have
